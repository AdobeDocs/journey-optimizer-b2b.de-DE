...
description: Erfahren Sie, wie Adobe Journey Optimizer B2B edition Content Credentials automatisch auf Bilder anwendet, die mit generativen KI-Tools generiert oder bearbeitet wurden, und was dies für Ihre Inhalte bedeutet.
...
## Aktionen, die Content Credentials betreffen {#cc-workflows}
...
Wenn Sie ein Bild mit Tools für generative KI in [!DNL Journey Optimizer B2B Edition] generieren oder bearbeiten, werden Content Credentials automatisch an dieses Bild angehängt, sodass keine Aktion Ihrerseits erforderlich ist.

### Bild erzeugen {#generate}

**_Example:_** Generieren eines Bannerbilds für eine E-Mail über eine Textaufforderung, die das gewünschte visuelle Element beschreibt. Content Credentials sind an das generierte Bild angehängt.

Wenn Sie ein neues Bild über eine Textaufforderung, ein Referenzbild oder ein ähnliches Bild erstellen, wird Content Credentials immer angehängt.

### Beschneiden eines Bildes {#crop}

**_Examples:_**

* Beschneiden Sie ein generiertes Bannerbild, damit es auf eine Web-Seite passt. Die Content Credentials bleiben durch den Zuschnitt erhalten.
* Verwenden Sie ein hochgeladenes Stockfoto als E-Mail-Hintergrund und schneiden Sie es so zu, dass es auf den Bildschirm passt. Wenn das Stockfoto keine generativen KI-Informationen enthält, werden keine Content Credentials erstellt.

Wenn Sie eine Anpassung an eine Bilddatei vornehmen, z. B. sie auf die angeforderten Abmessungen zuschneiden, behält sie ihre Content Credentials nur bei, wenn sie bereits im Quellbild vorhanden waren. Beim Zuschneiden werden die Pixel des Bildes neu erstellt, wodurch normalerweise diese Content Credential entfernt wird. Daher liest der KI-Assistent das Bild vor dem Zuschneiden aus dem Quellbild, erstellt es dann neu und fügt es erneut an das zugeschnittene Ergebnis an. Beim Zuschneiden selbst wird keine neue generative KI-Aktion hinzugefügt, sondern die vorhandene beibehalten.

### Hinzufügen einer Textüberlagerung

**_Example:_** Erstellen einer Werbe-Überschrift als Textüberlagerung auf einem generierten Hintergrundbild für eine Landingpage. Die Content Credentials aus dem Hintergrundbild werden beibehalten.

Wenn Sie generierten Text über einem Hintergrundbild rendern, werden Content Credentials nur dann an das resultierende Bild angehängt, wenn das Hintergrundbild bereits Content Credentials enthält. Beim Rendern der Überlagerung wird ein neues Bild erstellt, sodass das Bildbearbeitungs-Tool die Content Credentials aus dem Hintergrund liest und sie erneut an das Ergebnis anhängt. Der Überlagerungsschritt fügt keine neue generative KI-Aktion hinzu.

### Überlagern eines Bildes

**_Examples:_**

* Erstellen Sie eine E-Mail-Kopfzeile, indem Sie ein generiertes Produktbild mit einem generierten Hintergrund kombinieren. Das Ergebnis ist Content Credentials, das beide generativen KI-Quellen widerspiegelt.
* Kombinieren Sie zwei hochgeladene Markenfotos zu einem Collage-Bild. Da keines der Quellbilder über eine generative KI-Aktion verfügt, werden Content Credentials nicht erstellt.

Wenn Sie zwei oder mehr Bilder zusammenführen und eines der Quellbilder Content Credentials enthält, werden diese im kombinierten Bild beibehalten und zu einem einzigen Content Credentials-Metadatenelement zusammengeführt. Beim Zusammensetzen wird ein neues Bild aus den Quellen erstellt, wodurch diese Content Credentials normalerweise entfernt werden. Die Bildbearbeitungswerkzeuge lesen jedoch jedes Bild, bevor sie es zusammensetzen, und erstellen dann ein einziges kombiniertes Content Credentials-Element, das jede Quelle auflistet, die zu einer generativen KI-Aktion beigetragen hat.
