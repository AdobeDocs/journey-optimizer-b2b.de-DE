...
description: Verwenden Sie die Fähigkeit zur Zielgruppenerstellung in Journey Optimizer B2B Prime, um Personenlisten zu erstellen, Marketo Engage-Smart-Listen anzupassen und Listenregeln im Chat zu bearbeiten.
...
* **Lese-/Schreibvorgänge nach** - [!DNL Journey Optimizer B2B Prime]; liest [!DNL Marketo Engage] bei der Anpassung von Smart Lists

## Unterstützte Workflows {#workflows}

Der KI-Assistent unterstützt drei Workflows zur Erstellung von Zielgruppen und bestimmt anhand Ihrer Anfrage, welcher Workflow gelten soll. Wenn der Zweck mehrdeutig ist, werden Sie gefragt, bevor Sie fortfahren.
...
Bevor Sie etwas generieren, bestätigt der KI-Assistent alle vier folgenden Punkte. Es fragt nach allen, die fehlen - in einer einzigen Nachricht.

1. **Regeln/Kriterien** - Eine einfache Beschreibung dessen, wer in die Liste gehört.
1. **Name** - Wie nennt man die Liste?
1. **Location** - In welchem Programm die Liste leben soll. Geben Sie einen Programmnamen an, und der KI-Assistent findet ihn. Wenn mehrere Übereinstimmungen vorliegen, werden Sie aufgefordert, ihn auszuwählen.
1. **Type** - Dynamisch (filterbasiert, automatisch aktualisiert) oder Statisch (feste Mitgliedschaft). Dies ist erforderlich - der KI-Assistent wird es nicht erraten; wenn Sie es nicht angeben, wird er gefragt.

### Dynamische Listen {#dynamic-lists}

Für dynamische Listen schlägt der KI-Assistent proaktiv die Aufnahme von Personalisierungsattributen vor, um die Zielgruppenbestimmung umfassender zu gestalten. Diese Attribute sind **_standardmäßig enthalten - Sie schließen die Funktion ab, nicht_**:
...
Teilen Sie dem KI-Assistenten mit, ob Sie eine dieser Komponenten entfernen möchten, bevor der Vorgang fortgesetzt wird.

### Statische Listen {#static-lists}

* **Statisch, keine Kriterien** - Die Liste wird leer erstellt und kann manuell hinzugefügt werden.
* **Statisch aus Kriterien (eine Momentaufnahme)** - Der KI-Assistent erstellt den passenden Satz und kopiert die Personen in . Population ist asynchron - Der KI-Assistent bestätigt, dass die Liste erstellt wurde, merkt jedoch an, dass es einige Minuten dauern kann, bis Personen angezeigt werden. Sie wird nicht für sich in Anspruch nehmen, dass die Liste sofort bereit ist.

## Karte prüfen {#review-card}

Es wird nichts erstellt, bis Sie es genehmigen. Nachdem Sie Ihre Kriterien beschrieben haben, präsentiert der KI-Assistent eine interaktive Karte _People List Creation Review_ (für Anpassungen aus [!DNL Marketo Engage] Listen heißt die Karte _People List Conversion Review_).
...
Wenn Sie Kriterien beschreiben, übersetzt der KI-Assistent jede Bedingung in ein reales, bekanntes Attribut auf Personenebene. Auf der Überprüfungskarte können drei Ergebnisse angezeigt werden:
...
Wenn Sie darum bitten, Regeln für eine bereits vorhandene Liste zu ändern, legt der KI-Assistent fest, welche Liste und welcher Bearbeitungsmodus verwendet werden soll:
...
Der KI-Assistent fasst zusammen, was angewendet wird, gibt klar an, ob er hinzugefügt oder ersetzt wird, und bittet Sie, dies zu bestätigen, bevor er bestätigt wird. Nach der Anwendung werden die Gesamtzahl der Regeln und die Anzahl der hinzugefügten oder ersetzten Elemente angezeigt.
...
| **Listengröße** | Liest ungefähr die ersten ~1.000 Mitglieder jeder Liste aus. Bei größeren Listen zeigt der KI-Assistent an, dass die Ergebnisse unvollständig sind. |
| **Dynamische Listen als Entwurf** | Nicht vergleichbar: Eine Liste, die noch nicht veröffentlicht wurde, hat kein Live-Segment. Der KI-Assistent fordert Sie auf, die Liste zuerst zu veröffentlichen oder stattdessen eine statische Liste zu verwenden. |
...
|---|---|
| **Static-list adaption von[!DNL Marketo Engage]** | Sie können eine [!DNL Marketo Engage] statische Liste (oder eine E-Mail oder ein anderes nicht gefiltertes Asset) nicht in eine Personenliste anpassen. Statische Listen sind explizite Mitglieder-IDs und können nicht als Filter ausgedrückt werden. Der KI-Assistent fragt stattdessen nach einer Smart-Liste oder einer Smart-Kampagne. |
...
| **Tief verschachtelte UND/ODER-Logik** | Komplexe verschachtelte Logik kann auf eine UND/ODER-Ebene der obersten Ebene reduziert werden. Der KI-Assistent merkt dies an, wenn es dazu kommt. |
| **Namenskollisionen** | Nicht automatisch aufgelöst - Wenn der Name verwendet wird, fragt Sie der KI-Assistent nach einem anderen Namen, anstatt im Hintergrund ein Suffix anzuhängen. |
| **Genehmigung erforderlich** | Der KI-Assistent erstellt oder ändert eine Liste erst, wenn Sie auf **[!UICONTROL Fortfahren]** klicken, bestätigen oder eine eindeutige Genehmigung erteilen (_„genehmigt“_, _„sieht gut aus“_, _„Erstellen“_). |
